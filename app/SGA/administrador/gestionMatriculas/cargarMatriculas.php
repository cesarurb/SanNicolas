<?php
  ...
  $query = "select matricula.id, concat(alumno.nombres,' ',alumno.apellidos) as alumno, SECCION.seccion, SECCION.turno, matricula.fechaMatricula from matricula inner join alumno on alumno.id = matricula.idAlumno INNER JOIN SECCION ON SECCION.id = matricula.idSeccion where matricula.anio = '$anio' and matricula.idGrado = '$grado' and matricula.estado = 1";
