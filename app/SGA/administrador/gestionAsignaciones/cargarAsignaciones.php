<?php
  ...
  $query = "select ASIGNACION.id, concat(DOCENTE.nombres,' ',DOCENTE.apellidos) as docente, CURSO.nombre as curso, concat(GRADO.numeroGrado,'° ',GRADO.nivel) as grado, ASIGNACION.turno FROM sga_san_nicolas.asignacion INNER JOIN DOCENTE ON DOCENTE.id = ASIGNACION.idDocente INNER JOIN CURSO ON CURSO.id = ASIGNACION.idCurso INNER JOIN GRADO ON GRADO.id = CURSO.idGrado where ASIGNACION.estado = 1";
  //
